...
def extract_pdf_content(file_path, form_recognizer_client, use_layout=False): 
    ...
    poller = form_recognizer_client.begin_analyze_document(model, document = file_path)
...
def chunk_file_content(
    ...
    if file_format == "pdf":
        if form_recognizer_client is None:
            raise UnsupportedFormatError("form_recognizer_client is required for pdf files")
        content = extract_pdf_content(file_content, form_recognizer_client, use_layout=use_layout)
        cracked_pdf = True
    else:
    ...
    if metadata:
        for key in metadata.keys():
            content = key + " : " + metadata[key] + " " + content
